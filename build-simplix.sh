...
# SIMPLIX Version
SIMPLIX_VER=$(cat $DIR_ROOT/vendor/cos/common.mk | grep 'SIMPLIX_VER := *' | sed 's/.*= //')
...
echo -e "${CLR_BLD_GRN}Building Simplix. $SIMPLIX_VER for $DEVICE${CLR_RST}"
...
if [ "$FLAG_USER_BUILD" = 'y' ]; then
        lunch "simplix_$DEVICE-user"
else
        lunch "simplix_$DEVICE-userdebug"
fi
echo -e ""
